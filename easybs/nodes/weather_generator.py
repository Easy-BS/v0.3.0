...
# ----------------------- LangGraph node -----------------------
def weather_file(state: Dict[str, Any]) -> Dict[str, Any]:
    """
    LangGraph node:
    ...
    try:
        ...
        APPLY_SITE_LOCATION = False
        if APPLY_SITE_LOCATION:
            set_idf_location(idf_path, epw_meta, epw_path)
        else:
            log("Skipping SITE:LOCATION update (APPLY_SITE_LOCATION=False).")
        #set_idf_location(idf_path, epw_meta, epw_path)
